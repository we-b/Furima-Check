...
begin
    ...
    # 手動確認項目
    puts "【手動での確認が必要な項目一覧】"
    puts "画像が表示されており、画像がリンク切れなどになっていないこと（Herokuの仕様による画像のリンク切れは、要件未達に含まれない。Heroku上では一定時間経過すると画像が消える。）"
    puts "売却済みの商品は、画像上に「sold out」の文字が表示されるようになっていること"
    puts "何も編集せずに更新をしても画像無しの商品にならないこと"
    puts "商品名やカテゴリーの情報など、すでに登録されている商品情報は商品情報編集画面を開いた時点で表示されること（画像に関しては、表示されない状態で良い）"
    ...
    puts "電話番号にはハイフンは不要で、11桁以内であること（09012345678となる）"
