...
begin
    ...
    # puts "入力された販売価格によって、販売手数料や販売利益が変わること(JavaScriptを使用して実装すること)" # new_check_17
    # puts "-----------92期以降で追加で確認する項目-------------"
    puts "本番環境で投稿した商品画像が、S3に保存されること"
    # puts "ユーザー登録、商品出品、商品編集、商品購入時のエラーハンドリングの際、1つのエラーに対して同じエラーメッセージが重複して表示されないこと" # errors_messages_duplication_check("ユーザー新規登録",27)
    # puts "-----------101期以降で追加で確認する項目-------------"
    # puts "エラーハンドリングによって新規登録/ログインページに戻った場合でも、入力済みの項目（パスワード・パスワード確認用以外）は消えないこと" # form_with_model_option("nickname", @nickname, 28)
    # puts "エラーハンドリングによって出品ページに戻った場合でも、入力済みの項目（商品画像・販売手数料・販売利益以外）は消えないこと" # form_with_model_option("item-name", @item_name, 52)
    # puts "エラーハンドリングによって編集ページに戻った場合でも、入力済みの項目（画像・販売手数料・販売利益以外）は消えないこと" # form_with_model_option("item-name", @item_name, 80)
    # puts "エラーハンドリングによって購入ページに戻った場合でも、入力済みの項目（カード番号・有効期限・セキュリティコード以外）は消えないこと" # form_with_model_option("postal-code", @postal_code_error, 99)
    # puts "商品一覧ページにて商品情報をクリックすると、該当する商品の商品詳細表示ページへ遷移すること" # google_spreadsheet_input("◯",68)
    # puts "建物名は任意であること" # google_spreadsheet_input("◯",94)
    sleep 30000000000
